import React from "react";
import {
  View,
  TouchableOpacity,
  Image,
  Dimensions,
  StatusBar,
} from "react-native";
import Svg, { Path, Circle } from "react-native-svg";
import { colors } from "../../constants/colors";
import { Button, AppText } from "../../components";

const { width } = Dimensions.get("window");

// Standard Google Color Logo SVG
const GoogleIcon = () => (
  <Svg width={18} height={18} viewBox="0 0 24 24">
    <Path
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      fill="#4285F4"
    />
    <Path
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      fill="#34A853"
    />
    <Path
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
      fill="#FBBC05"
    />
    <Path
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 6.31l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      fill="#EA4335"
    />
  </Svg>
);

interface LoginScreenProps {
  onRegister: () => void;
  onLoginSuccess: () => void;
  onPhoneLogin: () => void;
}

export default function LoginScreen({ onRegister, onLoginSuccess, onPhoneLogin }: LoginScreenProps) {
  return (
    <View className="flex-1 bg-white">
      <StatusBar barStyle="light-content" />

      {/* Top Teal Container */}
      <View className="flex-1 bg-secondary-500 rounded-b-[40px] items-center pt-[60px] relative overflow-hidden">
        {/* Logo and App name header */}
        <View className="flex-row items-center gap-2 z-10">
          <AppText weight="black" className="text-[26px] text-white tracking-[0.5px]">
            MediGo
          </AppText>
        </View>

        {/* Doctor Illustration */}
        <Image
          source={require("../../../assets/images/login_doctor.png")}
          style={{
            position: "absolute",
            bottom: -50,
            width: width * 1.15,
            height: "92%",
          }}
          resizeMode="contain"
        />
      </View>

      {/* Bottom Container */}
      <View className="px-7 pt-[34px] pb-[34px] items-center">
        {/* Welcome Section */}
        <View className="items-center gap-1.5 mb-[34px]">
          <AppText weight="bold" className="text-[30px] text-neutral-900 text-center">
            Welcome Back
          </AppText>
          <AppText weight="regular" className="text-[15px] text-neutral-400 text-center">
            Sign in to continue
          </AppText>
        </View>

        {/* Action Buttons */}
        <View className="w-full gap-4 mb-[34px]">
          <Button
            title="Continue with Google"
            variant="outline"
            leftIcon={<GoogleIcon />}
            onPress={onLoginSuccess}
            size="lg"
            className="w-full bg-slate-100 border-transparent text-neutral-900 font-bold"
          />

          <Button
            title="Continue with Phone"
            variant="primary"
            size="lg"
            onPress={onPhoneLogin}
            className="w-full"
          />
        </View>

        {/* Register Footer */}
        <View className="flex-row justify-center items-center">
          <AppText weight="regular" className="text-[14px] text-neutral-400">
            New user?{" "}
          </AppText>
          <TouchableOpacity onPress={onRegister}>
            <AppText weight="bold" className="text-[14px] text-secondary-500">
              Register
            </AppText>
          </TouchableOpacity>
        </View>
      </View>
    </View>
  );
}
